'''
This is the unit test suite that tests for happy cases.
'''
...
def test_client_full_expected_path(client):
    """
    Test Case: Client File Uploaded to Digits Corrrectly Recognized
    - Author: Dylan
    - Purpose: Validate the application conducts it's functional requirements
        correctly when given the expected information.
    - Scenario:
        - Simulate a POST request to the `/prediction` route with correct
          image file data.
        - Assert the response status code is 200 (to indicate a valid request
          was processed).
        - Checks the image with the base model to get a digit output.
        - Verify that the response includes a prediction and the prediction
          is the expected number given the uploaded image
        - Verify that the prediction number matches the one generated by the
          model fixture.
    """
